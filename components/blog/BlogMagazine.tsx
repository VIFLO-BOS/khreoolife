...
export function BlogMagazine() {
  ...
  return (
    <>
      <div className="sticky top-[86px] z-20 mb-10 flex items-center justify-between gap-4 rounded-[22px] border border-ink/20 bg-white/95 p-3 backdrop-blur-xl max-lg:relative max-lg:top-auto max-lg:flex-col max-lg:items-start">
        <div className="flex flex-wrap gap-2">
          {magazineCategories.map((item) => (
            <button key={item} onClick={() => setFilter(item)} className={`rounded-full border border-ink px-3 py-2 text-[9px] font-black uppercase tracking-[.07em] transition-colors ${filter === item ? "bg-ink text-white" : "hover:bg-brand hover:text-white"}`}>{item}</button>
          ))}
        </div>
        <span className="pr-2 text-[10px] font-black uppercase tracking-[.08em] text-[#6f6972]">Magazine Index</span>
      </div>
              ...
              <motion.article layout key={article.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className={`card-border flex min-h-[440px] flex-col ${span} max-md:col-span-1 max-md:row-span-1`}>
                {article.image ? (
                  ...
                  <div className={`flex min-h-[250px] flex-1 items-center justify-center gap-3 p-6 ${article.tone === "purple" ? "bg-brand text-white" : "bg-yellow text-ink"}`}>
                    {article.tone === "purple" ? <span className="text-[120px] font-black tracking-[-18px]">ee</span> : <><span className="sticker -rotate-6">the miraculous life</span><span className="sticker rotate-6">supernatural life</span></>}
                  </div>
                )}
                <div className="bg-paper p-6">
                  <span className="pill-tag text-brand">{article.category}</span>
                  <h3 className="mt-4 font-display text-[30px] font-normal tracking-[-.04em]">{article.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-[#6f6972]">{article.excerpt}</p>
                  <button onClick={() => setSelected(article)} className="mt-5 border-b border-ink pb-1 text-[11px] font-black">Read preview ↗</button>
                </div>
              </motion.article>
            );
          })}
      ...
      <AnimatePresence>
        {selected ? (
          <motion.div className="fixed inset-0 z-[100]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <button className="absolute inset-0 bg-black/65" onClick={() => setSelected(null)} aria-label="Close article" />
            <motion.article initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.45, ease: [0.2,0.75,0.2,1] }} className="absolute bottom-0 right-0 top-0 w-[min(820px,94vw)] overflow-y-auto bg-paper p-14 max-md:p-7 max-md:pt-20">
              <button onClick={() => setSelected(null)} className="absolute right-5 top-5 grid size-11 place-items-center rounded-full border border-ink" aria-label="Close"><X className="size-5" /></button>
              <div className="text-[10px] font-black uppercase tracking-[.1em] text-brand">{selected.category}</div>
              <h2 className="display-title mt-8 text-[clamp(48px,6vw,82px)]">{selected.title}</h2>
              <p className="mt-5 text-xl leading-8 text-[#565057]">{selected.excerpt}</p>
              <div className="mt-10 font-display text-xl leading-9">{selected.body}</div>
              <div className="mt-12 flex justify-between gap-6 border-t border-ink pt-5 text-[10px] text-[#6f6972] max-sm:flex-col"><span>Editorial preview</span><span>Author, date and social-share UI can be connected when final content is supplied.</span></div>
            </motion.article>
          </motion.div>
        ) : null}
      </AnimatePresence>
