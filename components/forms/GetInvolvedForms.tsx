    ...
    <div className="grid grid-cols-[.72fr_1.28fr] overflow-hidden rounded-[30px] border border-ink bg-white max-lg:grid-cols-1">
      <aside className="flex min-h-[760px] flex-col justify-between bg-brand-dark p-12 text-white max-lg:min-h-[360px] max-md:p-7">
        <div>
          ...
          <h2 className="display-title text-[clamp(44px,5vw,70px)]">Take the next step.</h2>
          <p className="mt-5 max-w-[380px] text-sm leading-7 text-white/70">Choose the form that fits how you want to contribute. In V1, submissions are followed up manually by the Khreeolife team.</p>
        </div>
      ...
      <div className="p-12 max-md:p-7">
        <div className="mb-7 flex gap-2">
          <button onClick={() => { setTab("volunteer"); setSubmitted(false); }} type="button" className={`rounded-full border border-ink px-4 py-2 text-[10px] font-black ${tab === "volunteer" ? "bg-ink text-white" : ""}`}>Volunteer</button>
          <button onClick={() => { setTab("sponsor"); setSubmitted(false); }} type="button" className={`rounded-full border border-ink px-4 py-2 text-[10px] font-black ${tab === "sponsor" ? "bg-ink text-white" : ""}`}>Sponsor</button>
        </div>
        ...
        {submitted ? (
          <div className="grid min-h-[430px] place-items-center text-center">
            <div className="max-w-md">
              ...
              <h3 className="mt-5 font-display text-4xl font-normal">Thank you.</h3>
              <p className="mt-4 text-sm leading-7 text-[#6f6972]">Your interest has been captured in this prototype. The production site should submit the form to the selected backend, and the Khreeolife team will be in touch.</p>
              <button type="button" onClick={() => setSubmitted(false)} className="mt-6 rounded-full bg-brand px-5 py-3 text-[11px] font-black text-white">Submit another response</button>
            </div>
          </div>
        ) : tab === "volunteer" ? (
          <form onSubmit={submit}>
            <div className="mb-7 flex justify-between gap-5 border-b border-ink pb-4 max-sm:flex-col"><strong>Volunteer Application</strong><small className="text-[#6f6972]">Fields follow the V1 website architecture.</small></div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-7 max-md:grid-cols-1">
              ...
              <div className="col-span-full"><button className="rounded-full bg-brand px-6 py-3 text-[11px] font-black text-white transition-colors hover:bg-yellow hover:text-ink">Submit Volunteer Application ↗</button></div>
            </div>
          </form>
        ) : (
          <form onSubmit={submit}>
            <div className="mb-7 flex justify-between gap-5 border-b border-ink pb-4 max-sm:flex-col"><strong>Sponsor Application</strong><small className="text-[#6f6972]">Financial, In-Kind or Partnership interest.</small></div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-7 max-md:grid-cols-1">
              ...
              <div className="col-span-full"><button className="rounded-full bg-brand px-6 py-3 text-[11px] font-black text-white transition-colors hover:bg-yellow hover:text-ink">Submit Sponsor Application ↗</button></div>
            </div>
          </form>
        )}
        <p className="mt-7 text-[11px] leading-5 text-[#6f6972]">Backend application tracking is future scope in the architecture. This Next.js build currently demonstrates the front-end experience only.</p>
      </div>
    </div>
  );
}
...
function Field({ label, children, full = false }: { label: string; children: React.ReactNode; full?: boolean }) {
  return (
    <label className={full ? "col-span-full grid gap-2" : "grid gap-2"}>
      <span className="text-[9px] font-black uppercase tracking-[.1em]">{label}</span>
