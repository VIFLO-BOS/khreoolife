    ...
    <>
      <div className="grid grid-cols-2 gap-3 max-lg:grid-cols-1">
        <article className="card-border group relative bg-white">
          ...
          <span className="absolute right-5 top-5 grid size-12 place-items-center rounded-full bg-paper text-[10px] font-black">01</span>
          <div className="p-8"><div className="eyebrow mb-4 text-brand">Volunteer</div><h3 className="font-display text-4xl font-normal">Give your presence and ability.</h3><p className="mt-4 max-w-[560px] text-sm leading-7 text-[#6f6972]">Volunteers participate in transformational outreach and service across Community Development, Education and Missions.</p><button onClick={() => go("volunteer")} className="mt-6 rounded-full bg-ink px-5 py-3 text-[11px] font-black text-white transition-colors hover:bg-brand">Volunteer Application ↘</button></div>
        </article>
        <article className="card-border group relative bg-brand text-white">
          ...
          <span className="absolute right-5 top-5 grid size-12 place-items-center rounded-full bg-paper text-[10px] font-black text-ink">02</span>
          <div className="p-8"><div className="eyebrow mb-4 text-yellow">Sponsor</div><h3 className="font-display text-4xl font-normal">Put resources behind a project.</h3><p className="mt-4 max-w-[560px] text-sm leading-7 text-white/75">Sponsorship may be Financial, In-Kind or Partnership-based and can be directed toward specific projects.</p><button onClick={() => go("sponsor")} className="mt-6 rounded-full bg-white px-5 py-3 text-[11px] font-black text-ink transition-colors hover:bg-yellow">Sponsor Application ↘</button></div>
        </article>
      </div>
