  ...
  return (
    <>
      <PageHero
        ...
        aside={<><span className="sticker">serve · partner · support</span><p className="mt-5 text-sm text-white/75 leading-body">V1 submissions are received and followed up manually. Application tracking is future scope.</p></>}
      />
      ...
      <section className="bg-white py-24">
        <div className="site-container">
          ...
          <Reveal className="mt-10 grid grid-cols-4 border-t border-ink max-lg:grid-cols-2 max-md:grid-cols-1">
            {[["01","Choose","Volunteer or sponsor."],["02","Tell us","Complete the relevant interest form."],["03","Submit","Your details are received for follow-up."],["04","Connect","The team gets in touch manually."]].map(([n,title,copy],i) => <div key={n} className={`min-h-[200px] p-7 ${i<3 ? "border-r border-ink max-md:border-r-0" : ""} max-lg:border-b`}><span className="text-[10px] font-black text-brand leading-caption">{n}</span><h3 className="mt-10 font-display text-3xl font-normal leading-card">{title}</h3><p className="mt-2 text-sm text-[#6f6972] leading-body">{copy}</p></div>)}
          </Reveal>
        </div>
      </section>
    </>
